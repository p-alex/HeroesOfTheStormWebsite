...
app.post("/", (req, res) => {
    ...
    const apiID = "a7a5bf8652032e1b91c4a7982a0807d2";
    const units = "metric";
    const lang = "en";
    console.log(req.body.city);
    https.get(url + query + "&appid=" + apiID + "&units=" + units + "&lang=" + lang, response => {
        response.on("data", data => {
            const weatherData = JSON.parse(data);
            const temp = weatherData.main.temp;
            const weatherDesc = weatherData.weather[0].description;
            res.send("The temperature in " + req.body.city + " is " + temp + " degrees Celcius");
        });
    });
});

//a7a5bf8652032e1b91c4a7982a0807d2